...
/* ───── Constants ───── */
const ABOMAZEN_SERVICES = [
  "تسويق عقاري", "إدارة محتوى عقاري", "تصميم بروشورات عقارية", "تسويق عبر السوشيال ميديا العقارية", "خدمة مخصصة",
];
const WS_LIST = [
  { v: "WAITING", l: "انتظار", c: "#f59e0b", bg: "rgba(245,158,11,0.12)" },
  { v: "IN_PROGRESS", l: "قيد التنفيذ", c: "#f59e0b", bg: "rgba(245,158,11,0.12)" },
...
const TIER: Record<string, { l: string; c: string; bg: string }> = {
  ...
  LOYAL: { l: "دائم", c: "#f59e0b", bg: "rgba(245,158,11,0.08)" },
...
function InlineText({ value, onSave, style, placeholder }: { value: string; onSave: (v: string) => void; style?: React.CSSProperties; placeholder?: string }) {
  ...
  return <input autoFocus value={draft} onChange={e => setDraft(e.target.value)} onBlur={save}
    ...
    style={{ padding: "3px 6px", borderRadius: 4, border: "1px solid #f59e0b", background: "var(--surface)", color: "var(--text)", fontSize: 12, outline: "none", width: "100%", ...style }} />;
}
...
function PayButton({ remaining, projectId, onPay }: { remaining: number; projectId: string; onPay: (id: string, amount: number) => void }) {
  ...
  if (!editing) return (
    <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
      ...
      <button onClick={() => onPay(projectId, remaining)} style={{ padding: "2px 6px", borderRadius: 4, border: "none", background: "rgba(245,158,11,0.1)", color: "#f59e0b", fontSize: 10, fontWeight: 600, cursor: "pointer" }} title="Pay all">الكل</button>
    </div>
  );
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
      <input autoFocus type="number" min="1" max={remaining} step="1" value={amount} onChange={e => setAmount(e.target.value)}
        ...
        style={{ width: 60, padding: "2px 4px", borderRadius: 4, border: "1px solid #f59e0b", background: "var(--surface)", color: "var(--text)", fontSize: 11, outline: "none" }}
      />
      <button onClick={() => { if (amount) { onPay(projectId, Math.min(parseFloat(amount), remaining)); setEditing(false); setAmount(""); } }}
        style={{ padding: "2px 6px", borderRadius: 4, border: "none", background: "#f59e0b", color: "#fff", fontSize: 10, fontWeight: 600, cursor: "pointer" }}>✓</button>
    </div>
  );
}
...
function MonthHeader({ label, count, totalRevenue, totalCollected, totalRemaining, paidCount, collapsed, onToggle }: {
  ...
  return (
    <div style={{ borderRadius: collapsed ? 8 : "8px 8px 0 0", overflow: "hidden", border: "1px solid var(--border)" }}>
      <div onClick={onToggle} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 14px", background: "linear-gradient(135deg, rgba(245,158,11,0.08) 0%, rgba(245,158,11,0.02) 100%)", cursor: "pointer", borderBottom: collapsed ? "none" : "1px solid var(--border)" }}>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} width={12} height={12} style={{ color: "#f59e0b", transform: collapsed ? "rotate(-90deg)" : "rotate(0)", transition: "transform 0.2s", flexShrink: 0 }}><path d="M6 9l6 6 6-6" /></svg>
        <span style={{ fontSize: 13, fontWeight: 800, color: "var(--text)" }}>{label}</span>
        <span style={{ fontSize: 9, fontWeight: 600, padding: "2px 7px", borderRadius: 10, background: "rgba(245,158,11,0.1)", color: "#f59e0b" }}>
          {count} سجل
        </span>
        {paidCount > 0 && (
...
function OneTimeTab() {
  ...
  const fetchProjects = useCallback(async () => {
    ...
    try { const r = await fetch(`/api/abomazen/projects?${p}`); if (r.ok) setProjects(await r.json()); } catch {}
    setLoading(false);
  }, [search, wsFilter, psFilter]);

  const fetchUsers = async () => { try { const r = await fetch("/api/auth/me"); if (r.ok) setUsers([]); } catch {} };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const r = await fetch("/api/abomazen/projects", { method: "POST", headers: { "Content-Type": "application/json" },
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
        ...
        <button onClick={() => { setShowForm(true); resetForm(); }} style={{ display: "flex", alignItems: "center", gap: 5, padding: "9px 16px", borderRadius: 8, background: "#f59e0b", color: "#fff", border: "none", fontSize: 12, fontWeight: 600, cursor: "pointer", boxShadow: "0 2px 6px rgba(245,158,11,0.3)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 10, marginBottom: 16 }}>
        {[{ l: "الإجمالي", v: stats.total, c: "var(--text)" }, { l: "قيد التنفيذ", v: stats.inProgress, c: "#f59e0b" }, { l: "مكتمل", v: stats.completed, c: "#10b981" }, { l: "غير مدفوع", v: stats.unpaid, c: "#ef4444" }, { l: "العملاء", v: stats.clients, c: "#8b5cf6" }, { l: "الإيرادات", v: `${fmt(stats.revenue)} EGP`, c: "#f59e0b" }].map(s => (
              ...
              {!collapsed && (
                <div style={{ overflowX: "auto", border: "1px solid var(--border)", borderTop: "none", borderRadius: "0 0 6px 6px" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, minWidth: 1000 }}>
                    <thead>
                      <tr style={{ background: "linear-gradient(135deg, rgba(245,158,11,0.06) 0%, transparent 100%)" }}>
                          ...
                          <tr key={p.id} style={{
                            ...
                            <td style={{ padding: "5px 8px" }}>
                              <div style={{ display: "flex", flexWrap: "wrap", gap: 2 }}>
                                {p.services.map(s => <span key={s.id} style={{ padding: "1px 5px", borderRadius: 3, fontSize: 9, fontWeight: 600, background: "rgba(245,158,11,0.08)", color: "#f59e0b" }}>{s.name}</span>)}
          ...
          <div style={{ background: "var(--surface)", borderRadius: 14, maxWidth: 520, width: "95%", border: "1px solid var(--border)" }}>
            ...
            <form onSubmit={handleSubmit} style={{ padding: "16px 20px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 4 }}>
                    {ABOMAZEN_SERVICES.map(s => (
                      <button key={s} type="button" onClick={() => setForm(f => ({ ...f, customServiceText: f.customServiceText === s ? "" : s }))}
                        style={{ padding: "4px 8px", borderRadius: 5, border: form.customServiceText === s ? "1px solid #f59e0b" : "1px solid var(--border)", background: form.customServiceText === s ? "rgba(245,158,11,0.12)" : "transparent", color: form.customServiceText === s ? "#f59e0b" : "var(--muted)", fontSize: 10, fontWeight: 600, cursor: "pointer" }}>
                        {s}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <input value={form.designerName} onChange={e => setForm(f => ({ ...f, designerName: e.target.value }))} style={{ width: "100%", padding: "7px 10px", borderRadius: 6, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text)", fontSize: 12, outline: "none", marginTop: 3 }} />
                </div>
                <div style={{ gridColumn: "1 / -1" }}>
              ...
              <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 16 }}>
                ...
                <button type="submit" disabled={submitting} style={{ padding: "8px 18px", borderRadius: 8, border: "none", background: "#f59e0b", color: "#fff", fontSize: 12, fontWeight: 600, cursor: "pointer", opacity: submitting ? 0.6 : 1 }}>{submitting ? "جاري الحفظ..." : "حفظ"}</button>
              </div>
            </form>
          </div>
        </div>
...
function RecurringTab() {
  ...
  const fetchSubscriptions = useCallback(async () => {
    ...
    try { const r = await fetch(`/api/abomazen/subscriptions?${p}`); if (r.ok) setSubscriptions(await r.json()); } catch {}
    setLoading(false);
  }, [statusFilter]);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault(); setSubmitting(true); setError("");
    try {
      const r = await fetch("/api/abomazen/subscriptions", { method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const handlePay = async () => {
    ...
    try {
      const r = await fetch("/api/abomazen/subscription-invoices", { method: "POST", headers: { "Content-Type": "application/json" },
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
        ...
        <button onClick={() => setShowForm(true)} style={{ display: "flex", alignItems: "center", gap: 5, padding: "9px 16px", borderRadius: 8, background: "#f59e0b", color: "#fff", border: "none", fontSize: 12, fontWeight: 600, cursor: "pointer", boxShadow: "0 2px 6px rgba(245,158,11,0.3)" }}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>
          اشتراك جديد +
        </button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 16 }}>
        {[
          ...
          { l: "اشتراكات نشطة", v: subscriptions.filter(s => s.status === "ACTIVE").length, c: "#f59e0b" },
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              ...
              <tbody>
                {subscriptions.map(sub => {
                  ...
                  const currentInvoice = sub.invoices[0];
                      ...
                      <td style={{ padding: "6px 10px" }}>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 2 }}>
                          {(() => { try { return JSON.parse(sub.services); } catch { return [sub.services]; } })().map((s: string, i: number) => (
                            <span key={i} style={{ padding: "1px 5px", borderRadius: 3, fontSize: 9, fontWeight: 600, background: "rgba(245,158,11,0.08)", color: "#f59e0b" }}>{s}</span>
                          ))}
                        </div>
                      </td>
                      <td style={{ padding: "6px 10px", fontWeight: 700, color: "#f59e0b", direction: "ltr" }}>{fmt(sub.monthlyFee)} EGP</td>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={e => { if (e.target === e.currentTarget) { setPayModal(null); setPayAmount(""); } }}>
          <div style={{ background: "var(--surface)", borderRadius: 14, maxWidth: 400, width: "95%", border: "1px solid var(--border)" }}>
            ...
            <div style={{ padding: "16px 20px" }}>
              ...
              <input type="number" min="1" max={payModal.invoice.amount - payModal.invoice.paidAmount} value={payAmount} onChange={e => setPayAmount(e.target.value)}
                style={{ width: "100%", padding: "8px 10px", borderRadius: 6, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text)", fontSize: 13, outline: "none", marginTop: 4, boxSizing: "border-box" }} />
              <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 16 }}>
                <button onClick={() => { setPayModal(null); setPayAmount(""); }} style={{ padding: "8px 16px", borderRadius: 8, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text)", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>إلغاء</button>
                <button onClick={handlePay} disabled={!payAmount || parseFloat(payAmount) <= 0} style={{ padding: "8px 18px", borderRadius: 8, border: "none", background: "#10b981", color: "#fff", fontSize: 12, fontWeight: 600, cursor: "pointer", opacity: !payAmount || parseFloat(payAmount) <= 0 ? 0.5 : 1 }}>تأكيد الدفع</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showForm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={e => { if (e.target === e.currentTarget) setShowForm(false); }}>
          <div style={{ background: "var(--surface)", borderRadius: 14, maxWidth: 480, width: "95%", border: "1px solid var(--border)" }}>
            <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h3 style={{ fontSize: 15, fontWeight: 700, color: "var(--text)", margin: 0 }}>اشتراك جديد</h3>
            ...
            <form onSubmit={handleCreate} style={{ padding: "16px 20px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                ...
                <div style={{ gridColumn: "1 / -1" }}>
                  ...
                  <input value={form.services} onChange={e => setForm(f => ({ ...f, services: e.target.value }))} placeholder="تسويق عقاري, إدارة محتوى" style={{ width: "100%", padding: "7px 10px", borderRadius: 6, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text)", fontSize: 12, outline: "none", marginTop: 3 }} />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input required type="date" value={form.startDate} onChange={e => setForm(f => ({ ...f, startDate: e.target.value }))} style={{ width: "100%", padding: "7px 10px", borderRadius: 6, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text)", fontSize: 12, outline: "none", marginTop: 3 }} />
                </div>
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: "var(--muted)" }}>يوم الفاتورة الشهري</label>
                  <input type="number" min="1" max="31" value={form.billingDay} onChange={e => setForm(f => ({ ...f, billingDay: e.target.value }))} style={{ width: "100%", padding: "7px 10px", borderRadius: 6, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text)", fontSize: 12, outline: "none", marginTop: 3 }} />
                </div>
                <div style={{ gridColumn: "1 / -1" }}>
                  <label style={{ fontSize: 10, fontWeight: 600, color: "var(--muted)" }}>ملاحظات</label>
                  <textarea rows={2} value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} style={{ width: "100%", padding: "7px 10px", borderRadius: 6, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text)", fontSize: 12, outline: "none", marginTop: 3, resize: "vertical" }} />
...
/*                   MAIN PAGE (TABS)                    */
/* ═══════════════════════════════════════════════════════ */
export default function AbomazenClientsPage() {
    ...
    <div>
      ...
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 24, fontWeight: 800, color: "var(--text)", margin: 0 }}>
          إدارة العملاء <span style={{ fontSize: 14, fontWeight: 500, color: "var(--muted)", marginLeft: 6 }}>ABOMAZEN</span>
        </h1>
        <p style={{ fontSize: 12, color: "var(--muted)", margin: "4px 0 0" }}>إدارة المشاريع والاشتراكات الشهرية — تسويق عقاري</p>
      </div>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 20, background: "var(--surface)", padding: 4, borderRadius: 10, border: "1px solid var(--border)", width: "fit-content" }}>
        {[
          { key: "one_time" as const, label: "📋 لمرة واحدة", labelEn: "One-time" },
          { key: "recurring" as const, label: "🔄 اشتراكات شهرية", labelEn: "Subscriptions" },
        ].map(t => (
          <button key={t.key} onClick={() => setActiveTab(t.key)} style={{
            padding: "8px 18px", borderRadius: 8, border: "none", display: "flex", alignItems: "center", gap: 6,
            background: activeTab === t.key ? "#f59e0b" : "transparent",
            color: activeTab === t.key ? "#fff" : "var(--muted)", fontSize: 12, fontWeight: 600, cursor: "pointer",
            transition: "all 0.15s",
          }}>
            <span>{t.label}</span>
            <span style={{ fontSize: 10, opacity: 0.7 }}>{t.labelEn}</span>
          </button>
        ))}
      </div>

      {/* Tab Content */}
      {activeTab === "one_time" && <OneTimeTab />}
